...
class TwitterClient: BDBOAuth1SessionManager {
    ...
    func userTimelineWithParams(params: NSDictionary?, completion: (tweets: [Tweet]?, error: NSError?) -> ()) {
        ...
        GET("1.1/statuses/user_timeline.json?screen_name=\(params!["screen_name"] as! String)", parameters: params, success: { (operation: NSURLSessionDataTask, response: AnyObject?) -> Void in
            
                var userTweets = Tweet.tweetsWithArray(response as! [NSDictionary])
            
                completion(tweets: userTweets, error: nil)
    ...
    func getStatus(params: NSDictionary?, completion: (returns: AnyObject?, error: NSError?) -> ()) {
        GET("1.1/application/rate_limit_status.json?resources=help,users,search,statuses", parameters: params, success: { (operation: NSURLSessionDataTask, response: AnyObject?) -> Void in
                ...
                completion(returns: response, error: nil)
            }, failure: { (operation: NSURLSessionDataTask?, error: NSError) -> Void in
                completion(returns: nil, error: error)
        })
